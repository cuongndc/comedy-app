...
<template>
  <div v-for="manga in mangas" :key="manga.slug">
    <NuxtLink class="flex items-center p-4" :to="useMangaDetailPagePath(manga.slug)">
      <div class="relative">

        <SharedImg
            loading="lazy"
            class="rounded-xl w-[125px] h-[168px] object-cover"
            :src="manga.thumbnail"
            fil="fill"/>
      ...
      <div class="px-5 h-[168px]" style="width: calc(100% - 102px)">
        ...
        <p class="text-gray-custom text-xl">
          Lượt xem: {{ manga.view }}
        </p>
        <p class="text-gray-custom text-xl mt-4">
