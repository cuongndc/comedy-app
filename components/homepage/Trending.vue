...
<template>
  <div class="px-4 bg-contain">
    <div>
      <NuxtLink href="/">
        <nuxt-img format="png" quality="100" class="w-full" src="/icons/widgets/trend/img-header.svg"
  ...
  fill: 'row',
        ...
        <SwiperSlide v-for="content in record.content" :key="content.slug" class="h-[130px]">
          <div class="flex items-center p-5" @click="navigatorComicPreview(content.slug, content._id)">
            <shared-mee-toon-img fil="cover" class="aspect-[3/4] w-[75px] h-full relative rounded-2xl" :width="75"
              :height="100" sizes="sm:75px 2sm:100px sm:150px md:150px" :src="content.verticalLogo" />
